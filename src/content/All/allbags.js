import img1 from '../../assets/item/Resize_Website4_0000__DSC5304.jpg';
import img2 from '../../assets/item/fea739d1-03fb-41d0-829d-bba6819fa7e7.jpg';
import img3 from '../../assets/item/94accc60-90d1-44e5-8794-d54c6cd4591b.jpg';
import img4 from '../../assets/item/Resize_Website4_0003_Layer_2.jpg';
import img5 from '../../assets/item/Shoe_0003_Layer_79.jpg';
import img6 from '../../assets/item/4156a422-422a-49ee-bd11-24d0397e2f74.jpg';
import img7 from '../../assets/item/361070be-11e8-426c-a92a-13735e4a9b15.jpg';
import img8 from '../../assets/item/bc7481fd-22ff-4f8b-b2b4-bde0cef57867.jpg';
import img9 from '../../assets/item/3146d107-90ce-4265-8505-0e9400aa812a.jpg';
import img10 from '../../assets/item/a50cbae0-1371-4a72-bd7f-c44d5c68db5a.jpg';
import img11 from '../../assets/item/15.jpg';
import img12 from '../../assets/item/39.jpg';
import img13 from '../../assets/item/_0001__DSC5788_efy9xvz.jpg';
import img14 from '../../assets/item/guide_0000__DSC5744_sEtdVaH.jpg';
import img15 from '../../assets/item/guide_0000__DSC5614_d4wN3Ho.jpg';
import img16 from '../../assets/item/DSC6963_-_DSC6966_0000__DSC6963.jpg';
import img17 from '../../assets/item/GfriendTan_2_1000x.webp';
import img18 from '../../assets/item/DSC02472_1000x.webp';
import img19 from '../../assets/item/_DSC2004.jpg';
import img20 from '../../assets/item/_DSC9218.jpg';




// categoryImg
import img30 from '../../assets/item/Resize_Website4_0001_Layer_1.jpg';
import img31 from '../../assets/item/Resize_Website4_0000_Layer_5.jpg';
import img32 from '../../assets/item/Resize_Website4_0000__DSC5314.jpg';
import img33 from '../../assets/item/07992722-fffc-49ce-a2e6-7a12fc519f80.jpg';
import img34 from '../../assets/item/7a2b5053-afc7-430a-9efc-c0cebff6c483.jpg';
import img35 from '../../assets/item/25036851-d89d-49d6-978f-f13be04e1f24.jpg';
import img36 from '../../assets/item/7a08220d-b9cb-4b2e-afc0-74f8fe3e7e10.jpg';
import img37 from '../../assets/item/7dd7c496-a11b-4dac-936b-7ca4e1460d63.jpg';
import img38 from '../../assets/item/1.webp';
import img39 from '../../assets/item/_DSC2040.jpg';
import img40 from '../../assets/item/MitiOffWhite3_1000x.webp';
import img41 from '../../assets/item/NovellaNobleBlack_1_1000x.webp';
import img42 from '../../assets/item/DSC08575-Edit_1000x.webp';
import img43 from '../../assets/item/DSC08597-Edit_1000x.webp';
import img44 from '../../assets/item/DaisyBlack_2_1000x.webp';
import img45 from '../../assets/item/DSC08626-Edit_1000x.webp';










export const Bag = [
  {
    id: 1,
    category: "Bags",
    image: img1,
    name: " Classic Leather Messenger Bag",
    categoryImg: [img1, img30, img31],
    catetitle: "Messenger Bag",
    title: "Premium full‑grain leather with adjustable strap",
    oldprice: "$120",
    price: "$95000",
    discountPercent: "21%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Durable, stylish, and perfect for office use."
  },
  {
    id: 2,
    category: "Bags",
    image: img2,
    name: " Modern Crossbody Bag",
    categoryImg: [img2, img32, img33],
    catetitle: "Crossbody Bag",
    title: "Lightweight, waterproof material for daily use",
    oldprice: "$100",
    price: "$80",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Very comfortable and trendy design."
  },
  {
    id: 3,
    category: "Bags",
    image: img3,
    name: " Urban Laptop Bag",
    categoryImg: [img3, img34, img35],
    catetitle: "Laptop Bag",
    title: "Spacious compartments, ideal for office and travel",
    oldprice: "$135",
    price: "$110",
    discountPercent: "18%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Fits laptop perfectly, but slightly heavy."
  },
  {
    id: 4,
    category: "Bags",
    image: img4,
    name: " Elegant Travel Duffel",
    categoryImg: [img4, img36, img37],
    catetitle: "Duffel Bag",
    title: "Stylish and durable travel bag with multiple pockets",
    oldprice: "$150",
    price: "$120",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Good capacity, perfect for weekend trips."
  },
  {
    id: 5,
    category: "Bags",
    image: img5,
    name: " Premium Sling Bag",
    categoryImg: [img5, img30, img31],
    catetitle: "Sling Bag",
    title: "Compact and stylish, easy to carry",
    oldprice: "$85",
    price: "$68",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Very handy and lightweight for casual use."
  },
  {
    id: 6,
    category: "Bags",
    image: img6,
    name: " Outdoor Backpack",
    categoryImg: [img6, img32, img33],
    catetitle: "Backpack",
    title: "Durable polyester, perfect for hiking or school",
    oldprice: "$110",
    price: "$88",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Lots of storage, good for day trips."
  },
  {
    id: 7,
    category: "Bags",
    image: img7,
    name: " Leather Belt Bag",
    categoryImg: [img7, img34, img35],
    catetitle: "Belt Bag",
    title: "Elegant leather design with adjustable belt",
    oldprice: "$75",
    price: "$60",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Looks very classy and fits all sizes."
  },
  {
    id: 8,
    category: "Bags",
    image: img8,
    name: " Classic Briefcase",
    categoryImg: [img8, img36, img37],
    catetitle: "Briefcase",
    title: "Professional leather briefcase with laptop compartment",
    oldprice: "$140",
    price: "$112",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Excellent for office use, premium quality."
  },
  {
    id: 9,
    category: "Bags",
    image: img9,
    name: " Sports Gym Bag",
    categoryImg: [img9, img30, img31],
    catetitle: "Gym Bag",
    title: "Durable and spacious, perfect for sports equipment",
    oldprice: "$95",
    price: "$76",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Very roomy and durable, great for gym use."
  },
  {
    id: 10,
    category: "Bags",
    image: img10,
    name: " Weekend Travel Backpack",
    categoryImg: [img10, img32, img33],
    catetitle: "Backpack",
    title: "Comfortable straps and multiple compartments",
    oldprice: "$130",
    price: "$104",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Good for short trips, fits all essentials."
  },
  {
    id: 11,
    category: "Bags",
    image: img11,
    name: " Elegant Leather Shoulder Bag",
    categoryImg: [img11, img42, img44],
    catetitle: "Leather Bag",
    title: "Premium Italian Design — Stylish and Durable",
    oldprice: "$95",
    price: "$75",
    discountPercent: "21%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Soft leather, elegant look, perfect for daily use."
  },
  {
    id: 12,
    category: "Bags",
    image: img12,
    name: " Classic Handbag for Women",
    categoryImg: [img12, img38, img39],
    catetitle: "Handbag",
    title: "Fashionable Everyday Handbag with Gold Chain Strap",
    oldprice: "$85",
    price: "$68",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Perfect size and very stylish look."
  },
  {
    id: 13,
    category: "Bags",
    image: img13,
    name: " Modern Tote Bag",
    categoryImg: [img13, img44, img42],
    catetitle: "Tote Bag",
    title: "Spacious Tote Bag for Office or Travel",
    oldprice: "$110",
    price: "$90",
    discountPercent: "18%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Good quality but a bit large for casual use."
  },
  {
    id: 14,
    category: "Bags",
    image: img14,
    name: " Royal Black Purse",
    categoryImg: [img14, img43, img40],
    catetitle: "Purse",
    title: "Elegant Black Purse with Silver Detailing",
    oldprice: "$75",
    price: "$60",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Sleek design, matches with any outfit."
  },
  {
    id: 15,
    category: "Bags",
    image: img15,
    name: " Brown Handcrafted Bag",
    categoryImg: [img15, img38, img42],
    catetitle: "Leather Bag",
    title: "Handmade Leather Bag with Premium Finish",
    oldprice: "$100",
    price: "$85",
    discountPercent: "15%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Excellent craftsmanship and durable quality."
  },
  {
    id: 16,
    category: "Bags",
    image: img16,
    name: " Premium Sling Bag",
    categoryImg: [img16, img45, img39],
    catetitle: "Sling Bag",
    title: "Lightweight Shoulder Sling Bag for Women",
    oldprice: "$70",
    price: "$55",
    discountPercent: "21%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Compact and cute, perfect for short outings."
  },
  {
    id: 17,
    category: "Bags",
    image: img17,
    name: " Elegant Tan Handbag",
    categoryImg: [img17, img38, img44],
    catetitle: "Handbag",
    title: "Luxury Tan Handbag with Smooth Finish",
    oldprice: "$90",
    price: "$72",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Color and design are absolutely gorgeous."
  },
  {
    id: 18,
    category: "Bags",
    image: img18,
    name: " Soft Brown Leather Tote",
    categoryImg: [img18, img39, img43],
    catetitle: "Tote Bag",
    title: "Premium Soft Leather Tote Bag for Daily Use",
    oldprice: "$95",
    price: "$78",
    discountPercent: "18%",
    ratestar: "⭐⭐⭐⭐⭐",
    review: "Super spacious and elegant — highly recommended."
  },
  {
    id: 19,
    category: "Bags",
    image: img19,
    name: " Small Luxury Purse",
    categoryImg: [img19, img40, img41],
    catetitle: "Purse",
    title: "Compact Women’s Purse for Parties",
    oldprice: "$60",
    price: "$48",
    discountPercent: "20%",
    ratestar: "⭐⭐⭐⭐☆",
    review: "Very chic and stylish for evening use."
  },
  {
    id: 20,
    category: "Bags",
    image: img20,
    name: " Royal Blue Handbag",
    categoryImg: [img20, img44, img45],
    catetitle: "Handbag",
    title: "Elegant Royal Blue Bag with Golden Handle",
    oldprice: "$85",
    price: "$70",
    discountPercent: "18%",
    ratestar: "⭐⭐⭐☆☆",
    review: "Looks great, quality could be slightly better."
  }
];












